...
function errorhandler($type, $string, $file, $line, $context) {
	switch ($type) {
	case E_STRICT:
		return;
	default:		
		throw new \Exception("$line: $string");
	}
}

try {
	$distance = $_GET['d']; //inches
	$size = $_GET['s']; //inches
	$angle = $_GET['a']; //radians
} catch (\Exception $e) {
...
$fontsize = $size * 0.8;
$fontpos = array($corner1[0] + ($size-$fontsize)/2, $corner1[1] + $fontsize);
...
<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="{$px_width}px" height="{$px_height}px" viewBox="0 0 {$image_width} {$image_height}">
	<!-- the observer -->
	...
	<text x="{$fontpos[0]}" y="{$fontpos[1]}" font-family="Impact" font-size="{$fontsize}" stroke="none" fill="black">Aa</text>
